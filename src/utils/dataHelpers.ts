import { Expense, Category, ExpenseFilters as ExpenseFiltersType, ExpenseSummary, MonthlyData } from '../types';
...
export function filterExpenses(expenses: Expense[], filters: ExpenseFiltersType): Expense[] {
  return expenses.filter(expense => {
    ...
    if (filters.searchText && !expense.memo.toLowerCase().includes(filters.searchText.toLowerCase())) return false;
    return true;
  });
}

export function generateCategorySummary(expenses: Expense[], categories: Category[]): ExpenseSummary[] {
  const summary: Record<string, { amount: number; count: number; color: string }> = {};
